...
import com.jixiexiaoge.drivingassist.navigation.NaviWebSocketV2Client
import com.jixiexiaoge.drivingassist.navigation.NaviStreamManager
...
class MainActivityLifecycle(
    ...
    private suspend fun waitForDeviceAndStartXiaogeReceiver() {
        ...
        try {
            // 创建 WebSocket 客户端
            val wsClient = com.jixiexiaoge.drivingassist.data.CarrotWsClient()
            ...
            CoroutineScope(Dispatchers.Default).launch {
                wsClient.connectionState.collect { state ->
                    lifecycleScope.launch {
                        core.wsConnected.value = (state == com.jixiexiaoge.drivingassist.data.ConnectionState.CONNECTED)
                        // 断开连接时重置 IP 缓存，允许重新触发连接
                        if (state == com.jixiexiaoge.drivingassist.data.ConnectionState.DISCONNECTED) {
                    ...
                    lifecycleScope.launch {
                        ...
                        val xiaogeData = XiaogeVehicleData(
                            ...
                            carState = data.carState?.let { cs ->
                                com.jixiexiaoge.drivingassist.CarStateData(
                                    vEgo = cs.vEgo,
                            ...
                            modelV2 = data.modelV2?.let { mv ->
                                com.jixiexiaoge.drivingassist.ModelV2Data(
                                    lead0 = com.jixiexiaoge.drivingassist.LeadData(
                                        x = mv.leadX,
                                        y = 0f,
                                        v = mv.leadV,
                                        prob = mv.leadProb
                                    ...
                                    meta = com.jixiexiaoge.drivingassist.MetaData(
                            ...
                            systemState = data.selfdriveState?.let { ss ->
                                com.jixiexiaoge.drivingassist.SystemStateData(
                                    enabled = ss.enabled,
                                    active = ss.active
                                )
                            },
            ...
            core.networkManager.setOnDeviceIPUpdated { deviceIP ->
                if (deviceIP.isNotEmpty()) {
                    ...
                    // HTTP 参数客户端
                    core.carrotParamClient = com.jixiexiaoge.drivingassist.CarrotParamClient(deviceIP)
    ...
    private fun initializeNaviV2Client() {
        try {
            ...
            if (currentDeviceIp != null && currentDeviceIp.isNotEmpty()) {
                ...
                val xiaogeClient = com.jixiexiaoge.drivingassist.navigation.XiaogeTcpClient()
                ...
                core.networkManager.setOnDeviceIPUpdated { deviceIP ->
                    if (deviceIP.isNotEmpty() && core.naviV2Client != null) {
                        ...
                        if (core.xiaogeTcpClient == null) {
                            val xc = com.jixiexiaoge.drivingassist.navigation.XiaogeTcpClient()
